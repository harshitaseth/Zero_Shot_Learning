...
class MyDataset(data.Dataset):
	def __init__(self, data_path, split='TRAIN', input_type='spec', input_length=None, num_chunk=16, w2v_type='google'):
		...
		if split == 'TRAIN':
			...
			self.eval_ids =pickle.load(open(os.path.join(data_path, 'sample_val_ids.pkl'),"rb"))
		elif split == 'TEST':
	...
	def get_train_item(self, index):
		...
		data = self.dataset_mapping[train_id]
		tags = data["Tags"]
		...
		return tag_emb,neg_tag_emb, spec, instrument_label[0]
