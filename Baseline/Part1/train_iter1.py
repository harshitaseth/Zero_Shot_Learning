...
def get_dataloader(dataset):
    ...
    val_loader = DataLoader(dataset["val"], 
                          batch_size=1, shuffle=True, drop_last=False, num_workers=num_workers)
...
def train_model(loader, optimizer, model):
    ...
    for i, batch in tqdm(enumerate(loader)):
        ...
        # time_before_encoder = time.time()
        audio_features = audio_encoder(spec)
        # time_after_audioencoder = time.time()
        text_features = text_encoder(tag)
        # time_after_textencoder = time.time()
        ...
        audio_out.append(audio_features.cpu())
        text_out.append(text_features.cpu())
        ...
        time_after_bpt = time.time()
        # print("time_before_encoder",time_before_encoder)
        # print("time_after_audioencoder",time_after_audioencoder)
        # print("time_after_textencoder",time_after_textencoder)
        # print("time_after_model",time_after_model)
        # print("time_after_bpt",time_after_bpt)
    
    
    train_out = [audio_out,text_out]
    pkl.dump(train_out, open("train_features_20k_pretrained_encoders.pkl","wb"))
    import pdb;pdb.set_trace()
    del train_out
...
def val_model(val_loader,model):
    ...
    for i, batch in enumerate(val_loader):
        ...
        out = model(audio_features)
        audio_out.append(audio_features.cpu())
        text_out.append(text_features.cpu().detach())
        final_out.append(out.cpu().detach())
        # loss =  criterion(out, text_features)
        # batch_loss = loss.item()
        # running_loss += batch_loss
        import pdb;pdb.set_trace()
    val_out = [audio_out,text_out,final_out]
    import pdb;pdb.set_trace()
    pkl.dump(val_out, open("val_features_20k_pretrained_encoders.pkl","wb"))
    del val_out
...
if __name__ == '__main__':
    ...
    model_path = "/efs_storage/Harshita/Zero_Shot_Learning/Baseline/Part1/checkpoints/20k_sgd_1e-1/10.pth" 
