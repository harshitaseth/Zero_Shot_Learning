...
def train_model(loader, optimizer, model):
    ...
    for i, batch in tqdm(enumerate(loader)):
        ...
        spec, tag  = batch
        tag = tag.cuda()#.unsqueeze(0) ## ask Ilaria, why long?
        ...
        list_ = [0,5,10,15]
        dist = 1 - torch.matmul(out.squeeze(1), text_features.squeeze(1).T).flatten()
        positve_dist = dist[list_].mean()
        dist[list_] = 0.
        negative_dist = dist.mean()
        # loss =  positve_dist - negative_dist+0.1
        loss= margin + positve_dist - negative_dist
        ...
        positive_dist_out += positve_dist.item()
        negative_dist_out += negative_dist.item()
    ...
    return running_loss/len(loader), positive_dist_out/len(loader), negative_dist_out/len(loader)
...
def val_model(loader,model):
    ...
    for i, batch in enumerate(loader):
        ...
        spec, tag  = batch
        tag = tag.cuda() #.float().cuda()#.unsqueeze(0)
        spec = spec.float().cuda()#.unsqueeze(0)
        audio_features = spec
        text_features = tag
    ...
    return running_loss/len(val_loader),positive_dist_out/len(loader), negative_dist_out/len(loader)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        train_loss, train_pos_dis, train_neg_dis = train_model(train_loader, optimizer, model)
        val_loss_out, val_pos_dis, val_neg_dis=  val_model(val_loader,model)
        ...
        print("Epoch, Train, Val, lr", epoch, train_loss, val_loss, optimizer.param_groups[0]['lr'])
        print("Train_pos_dis, train_neg_dis",train_pos_dis, train_neg_dis)
        print("val_pos_dis, val_neg_dis",val_pos_dis, val_neg_dis)
        # import pdb;pdb.set_trace()
        # if mlflow:
